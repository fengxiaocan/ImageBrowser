...
public class ResolutionAdaptationUtils {
	...
	public static int getStatusHeightOnRReflect() {
		...
		try {
			...
			int height = Integer
					.parseInt(clazz.getField("status_bar_height").get(object).toString());
	...
	public static int getStatusHeight() {
		...
		int resourceId = Utils.getResources().getIdentifier("status_bar_height","dimen","android");
		if (resourceId > 0) {
			// 根据资源ID获取响应的尺寸值
	...
	public static int getNavigationBarHeight1() {
		...
		int contentHeight = AdapterUtils.getScreenHeight();
